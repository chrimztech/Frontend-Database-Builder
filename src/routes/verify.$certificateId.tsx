  ...
  queryOptions({
    ...
    queryFn: async (): Promise<CertRow> => {
      const { data, error } = await supabase
        .from("certificates")
        .select(
          "certificate_id,recipient_name,programme,issue_date,expiry_date,status,issuer_name,revoked_at,revoke_reason",
        )
...
function ResultShell({
  ...
  const config = {
    valid: {
      ...
      blurb:
        "The registry recognises this certificate as authentic and currently active.",
    },
    expired: {
      Icon: AlertCircle,
      color: "var(--gold)",
      ...
      blurb:
        "This certificate was genuinely issued, but its validity period has ended.",
    },
    revoked: {
      Icon: XCircle,
          ...
          <section className="surface-panel rounded-[1.8rem] p-6 sm:p-8">
            ...
            <div className="mt-8 grid gap-3">
              <SummaryItem
                ...
                value={cert?.certificate_id ?? id ?? "Unavailable"}
              />
              <SummaryItem
                label="Programme"
            ...
            {cert ? (
              <>
                ...
                <dl className="mt-8 space-y-3">
                  ...
                  <DetailRow
                    label="Certificate ID"
                    value={<span className="font-mono text-sm">{cert.certificate_id}</span>}
                  />
                  <DetailRow
                    label="Status"
                    value={<StatusPill variant={variant} />}
                  />
                ...
                <div className="mt-8 rounded-[1.35rem] border border-border/70 bg-white/72 p-5 shadow-[var(--shadow-soft)]">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">
                        Confirm the ID carefully, then contact the issuing office if the
                        certificate should exist but cannot be found here.
                      </p>
                    </div>
                  </div>
                </div>
